    ...
    async fn discover(&self) -> Result<Vec<Receiver>> {
        ...
        for device in devices {
            ...
            receivers.push(Receiver {
                ...
                port: device.port,
                model: device.model,
                // AirPlay stereo groups should be treated as one advertised
                // receiver. AirSend currently does not expose group metadata,
                // so this remains a display hint rather than routing behavior.
...
/// Deterministic backend for service and UI development.
#[derive(Default)]
...
impl AirPlayStream for MockStream {
    async fn send_pcm(&mut self, _samples: Vec<i16>, _sample_rate: u32, _channels: u8) -> Result<()> { Ok(()) }
    async fn set_volume(&mut self, volume: f32) -> Result<()> { self.volume = volume.clamp(0.0, 1.0); Ok(()) }
